...
} from "@/components/velt-comments-dynamic";
...
export default function Home() {
  ...
  const handleDragStart = (event: DragStartEvent) => {
    ...
    if (active.id.toString().startsWith("list-")) {
      const list = boardData.lists.find((list) => list.id === active.id);
            ...
            <DragOverlay>
              {activeList ? (
                <div className="w-72 sm:w-80 opacity-50">
                  <Card className="cursor-grabbing shadow-lg bg-muted/50">
                    <CardContent className="p-3 sm:p-4">
                      <div className="flex items-center justify-between mb-4">
                        <h3 className="font-semibold text-foreground text-sm sm:text-base truncate flex-1 mr-2">
                          {activeList.title}
                        </h3>
                      </div>
                      <div className="space-y-3 mb-4 min-h-[100px]">
                        {activeList.cards.slice(0, 3).map((card: any) => (
